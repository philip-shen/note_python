...
'''
[
{'name': 'food-11/testing/2315.jpg', 'href': '/dav/food-11/testing/2315.jpg', 'size': 65956, 
'created': datetime.datetime(2023, 4, 28, 16, 35, 27, tzinfo=tzutc()), 
'modified': datetime.datetime(2023, 4, 28, 16, 35, 27, tzinfo=datetime.timezone.utc), 'content_language': None, 
'content_type': 'image/jpeg', 'etag': '"101a4-5fa680f7e6e01"', 'type': 'file', 'display_name': None}, 
{'name': 'food-11/testing/0832.jpg', 'href': '/dav/food-11/testing/0832.jpg', 'size': 42239, 
'created': datetime.datetime(2023, 4, 28, 16, 42, 4, tzinfo=tzutc()), 
'modified': datetime.datetime(2023, 4, 28, 16, 42, 4, tzinfo=datetime.timezone.utc), 'content_language': None, 
'content_type': 'image/jpeg', 'etag': '"a4ff-5fa68272e0fac"', 'type': 'file', 'display_name': None},


{'name': 'food-11/testing/0471.jpg', 'href': '/dav/food-11/testing/0471.jpg', 'size': 33168, 
'created': datetime.datetime(2023, 4, 28, 16, 40, 22, tzinfo=tzutc()), 
'modified': datetime.datetime(2023, 4, 28, 16, 40, 22, tzinfo=datetime.timezone.utc), 'content_language': None, 
'content_type': 'image/jpeg', 'etag': '"8190-5fa68211c8b90"', 'type': 'file', 'display_name': None}]
'''
def webdav_ls(fs: WebdavFileSystem, WebdavSrvDir: str, opt_verbose='OFF'):
    list_fs_ls = fs.ls(WebdavSrvDir, detail=True)    

    for fs_ls in list_fs_ls:
        logger.info(f'fs_ls: {fs_ls}') if opt_verbose.lower() == 'on' else None

        for key, value in fs_ls.items():
            if fs_ls[key] == 'directory':
                logger.info('directory: {}'.format(fs_ls['name']))
                logger.info( '{}'.format(fs.ls(fs_ls['name'], detail=True)) ) if fs.ls(fs_ls['name'], detail=True) != [] else None
            
            elif fs_ls[key] == 'file':
                logger.info( 'file: {}'.format(fs_ls['name'])) if fs_ls['name'] != [] else None

def list_files(data_path):
    p = pathlib.Path(data_path).glob('**/*')
    list_files = [x for x in p if x.is_file()]
    num_list_files= len(list_files)

    return num_list_files, list_files
"""
解決 Python TypeError: a bytes-like object is required, not ‘str’
https://www.ltsplus.com/python/%E8%A7%A3%E6%B1%BA-python-typeerror-a-bytes-like-object-is-required-not-str
...
def upload_fileobj(
    ...
    file = storage_dir / upload_fname
    length = file.write_bytes(file.read_bytes())
    ...
    with file.open(mode="rb") as fobj:
        ...
        if opt_verbose.lower() == 'on':
            ...
            logger.info(f'webdav_dir: {webdav_dir}')
        fs.upload_fileobj(fobj, webdav_dir, size=size)
...
if __name__ == '__main__':
    ...
    fs = WebdavFileSystem(webdav_url, auth=(auth_username, auth_api_key))

    #webdav_ls(fs, json_data["path_dataset"][0]["path_data_testing"], opt_verbose='off' )    

    #num_all_files ,llst_all_files = list_files(dir_data_training)    
    #num_all_files ,llst_all_files = list_files(dir_data_testing)    
    #[logger.info(f'file: {file}') for file in llst_all_files]
    #logger.info(f'num_all_files: {num_all_files}')
    
    '/home/philphoenix/projects/food-11/testing/0001.jpg'
    upload_fileobj(dir_data_testing, '0001.jpg', 
                   fs, json_data["path_dataset"][0]["path_data_testing"], 
                   with_size= True, wrap_fobj= True,
                   opt_verbose= 'on',
                   )
    est_timer()