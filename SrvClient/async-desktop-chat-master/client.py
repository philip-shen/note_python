...
async def client(srv_ip):
    await asyncio.wait([websocket_reading(srv_ip), gui_application()])

def main(srv_ip):
    loop = asyncio.get_event_loop()
    loop.run_until_complete(client(srv_ip))
    loop.close()

if __name__ == '__main__':
    #print('started')
    main(srv_ip='192.168.3.187')
    #print('ended')
