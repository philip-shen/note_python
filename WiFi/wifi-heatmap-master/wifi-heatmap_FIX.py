...
df = pd.read_csv('input/mapping.csv',header=0)
...
def grid_plots():
    ...
    for beacon, i in zip(s_beacons, range(len(s_beacons))):
        ...
        if interpolate:
            ...
            rbf = Rbf(df['Drawing X'], df['Drawing Y'], df[beacon],
                    function='linear')
            ...
            z = ml.griddata(df['Drawing X'], df['Drawing Y'], df[beacon], x, y)
...
def max_plot():
    ...
    max_rssi = []
    for index , i in df[s_beacons].iterrows():
        max_rssi.append(max(i))

    pp.title("Maximum RSSI seen for each beacon")

    if interpolate:
        # Interpolate the data
        rbf = Rbf(df['Drawing X'], df['Drawing Y'], max_rssi, function='linear')
        print(rbf)
        ...
        z = ml.griddata(df['Drawing X'], df['Drawing Y'], max_rssi, x, y)
